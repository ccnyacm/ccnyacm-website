...
export async function getMLHEventsByDate(date: Date) {
  try {
    // reset time
    date.setHours(0);
    date.setMinutes(0);
    ...
    const response = await fetch('https://mlh-events.now.sh/na-2020');
    const hackathons = await response.json();
    // filter hackathons to make sure we return only those within the date range
    const filteredHackathons = [];
    for (let i = 0; i < hackathons; i++) {
      const { isHishSchool, startDate } = hackathons[i];
      const tempDate = new Date(startDate);
      if (!isHishSchool && (tempDate >= date && tempDate < nextDay)) {
        filteredHackathons.push(hackathons[i]);
      }
    }
    return filteredHackathons;
